  ...
  for cnt in range(1, 10):
    curriculum.append(
        TaskSpec(
            ...
            sampling_weight=100 - cnt * 8,  # TODO 10
        )
    )
...
# find resource tiles
for resource in m.Harvestable:  # 这里只需要水和树在视野内
  if resource is nmmo.lib.material.Water or resource is nmmo.lib.material.Foilage:
      continue
  curriculum.append(
      TaskSpec(
          eval_fn=CanSeeTile,
          eval_fn_kwargs={"tile_type": resource},
          sampling_weight=5,  # TODO 10
      )
  )
...
curriculum.append(
    TaskSpec(
        eval_fn=CanSeeTile,
        eval_fn_kwargs={"tile_type": nmmo.lib.material.Water},
        sampling_weight=30,  # TODO 10
    )
)
...
curriculum.append(
    TaskSpec(
        eval_fn=CanSeeTile,
        eval_fn_kwargs={"tile_type": nmmo.lib.material.Foilage},
        sampling_weight=30,  # TODO 10
    )
)
...
for skill in SKILLS:
  ...
  for level in LEVEL_GOAL[1:]:
    # since this is an agent task, num_agent must be 1 由于这是一个代理任务，num_agent必须为1
    curriculum.append(
        TaskSpec(
            eval_fn=AttainSkill,
...
for num_tick in STAY_ALIVE_GOAL:
  curriculum.append(
      TaskSpec(eval_fn=TickGE, eval_fn_kwargs={"num_tick": num_tick}, sampling_weight=num_tick / 10))  # TODO 这里增加权重 1

# occupy the center tile, assuming the Medium map size
# TODO: it'd be better to have some intermediate targets toward the center 最好把一些中间目标对准中心
  ...
  for cnt in EVENT_NUMBER_GOAL:
    curriculum.append(
        TaskSpec(
            ...
            sampling_weight=30 - round(cnt / 2),  # TODO 这里的权重增加一下？  5
        )
    )

# TODO hoarding gold 囤积黄金 -- evaluated on the current gold
...
# TODO making profits by trading -- only buying and selling are counted 通过交易获利--只计算买卖
for amount in EVENT_NUMBER_GOAL:
  curriculum.append(
      TaskSpec(
...
# managing inventory space 管理库存空间
...
for space in [2, 4, 8]:  # TODO 库存多不一定是好事吧
  curriculum += [
      TaskSpec(
...
# own item, evaluated on the current inventory  获取物品
...
# equip item, evaluated on the current inventory and equipment status  装备物品
  ...
  for level in LEVEL_GOAL:
    ...
    curriculum.append(
        TaskSpec(
            eval_fn=EquipItem,
            eval_fn_kwargs={"item": item, "level": level, "num_agent": 1},
            sampling_weight=10 - level if level < 10 else 1,
        )
    )


# consume items (ration, potion), evaluated based on the event log  消耗物品
...
# harvest items, evaluated based on the event log 收获项目，根据事件日志进行评估
...
# list items, evaluated based on the event log 列出项目，根据事件日志进行评估
for item in ALL_ITEM:
  for level in LEVEL_GOAL:
    # agent task
    for quantity in ITEM_NUM_GOAL:
...
# buy items, evaluated based on the event log 购买项目，根据事件日志进行评估
for item in ALL_ITEM:
  for level in LEVEL_GOAL:
    # agent task
    for quantity in ITEM_NUM_GOAL:
