...
class StatPostprocessor(pufferlib.emulation.Postprocessor):
    ...
    def reward_done_info(self, reward, done, info):
        ...
        unique_events = extract_unique_event(log, self.env.realm.event_log.attr_to_col)
        self._curr_unique_count = len(unique_events)
        ...
        self._update_stats(agent)  # 更新状态，例如采集等级提高
...
def extract_unique_event(log, attr_to_col):
    ...
    # idx = log[:, attr_to_col["event"]] == 3
    # ssss1 = log[idx, attr_to_col["event"]]
    # bbbb = attr_to_col["event"]
    # ssss = log[:, attr_to_col["event"]]
    # ssss2 = log[[False], attr_to_col["event"]]
    # ssss3 = log[[True], attr_to_col["event"]]
    ...
    for code, attrs in cols_to_ignore.items():
        idx = log[:, attr_to_col["event"]] == code  # TODO ?
        for attr in attrs:
